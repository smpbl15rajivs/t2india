...
const EnhancedSearchResults = ({ query, onClose }) => {
  ...
  const analyzeQuery = async (query) => {
    setLoading(true)
    try {
      const response = await fetch('https://vgh0i1coy9o9.manus.space/api/search', {
        method: 'POST',
        headers: {
  ...
  const submitContact = async () => {
    ...
    try {
      const response = await fetch('https://vgh0i1coy9o9.manus.space/api/submit-contact', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4">
        <div className="text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-800 mb-2">🔍 Custom Travel Request</h3>
          <p className="text-gray-600 mb-4">
            Thank you for your inquiry: <strong>"{query}"</strong>
          </p>
          <p className="text-sm text-blue-600 font-medium mb-6">
            T2India will revert in 4 working days with a personalized itinerary.
          </p>
